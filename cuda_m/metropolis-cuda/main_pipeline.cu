...
#include "kernel_metropolis_pipeline.h"
  ...
  for (int trial = 0; trial < atrials; ++trial) {
    ...
    for(int p = 0; p < apts; ++p) {
      ...
      for(int i = 0; i < ams; ++i) {
        ...
        cudaEvent_t event[(ar)];  // NUM = number of (A,B) pairs
        for (int i = 0; i < (ar); ++i) {
        ...
        for(int k = 0; k < ar; ++k) {
          ...
          kernel_metropolis<<< mcgrid, mcblock, 0, stream1 >>>(N, L, mdlat[k], dH, h, 
              -2.0f/aT[atrs[k].i], apcga[k], apcgb[k], 0, true);

              cudaEventRecord(event[k], stream1);
              cudaStreamWaitEvent(stream2, event[k], 0);

          kernel_metropolis<<< mcgrid, mcblock, 0, stream2 >>>(N, L, mdlat[k], dH, h, 
                -2.0f/aT[atrs[k].i], apcga[k], apcgb[k], 0, false);
        }

        cudaDeviceSynchronize();
        cudaCheckErrors("mcmc: kernel metropolis white launch");

        cudaEvent_t event2[(ar)];  // NUM = number of (A,B) pairs
        for (int i = 0; i < (ar); ++i) {
        ...
        for(int k = 0; k < ar; ++k) {
          kernel_metropolis<<< mcgrid, mcblock, 0, stream1>>>(N, L, mdlat[k], dH, h, 
              -2.0f/aT[atrs[k].i], apcga[k], apcgb[k], 1, true);

              cudaEventRecord(event2[k], stream1);
              cudaStreamWaitEvent(stream2, event2[k], 0);

              kernel_metropolis<<< mcgrid, mcblock, 0, stream2>>>(N, L, mdlat[k], dH, h, 
                -2.0f/aT[atrs[k].i], apcga[k], apcgb[k], 1, false);
        }
