...
int main(int argc, char* argv[]) {
  ...
  float* vali_phi_sum_tmp;
  cudaMalloc((void**)&vali_phi_sum_tmp, sizeof(float) * num_topics * block_cnt);

  cudaStream_t stream1, stream2;
  cudaStreamCreateWithFlags(&stream1, cudaStreamNonBlocking);
  cudaStreamCreateWithFlags(&stream2, cudaStreamNonBlocking);

  cudaEvent_t event[(repeat)], event1[(repeat)];  // NUM = number of (A,B) pairs
  for (int i = 0; i < (repeat); ++i) {
      cudaEventCreate(&event[i]);
      cudaEventCreate(&event1[i]);
  }
  ...
  for (i = 0; i < repeat; i++) {
    ...
    if(i >= 2)
    {
      cudaStreamWaitEvent(stream1, event1[i - 2], 0);
    }
    EstepKernel<<<block_cnt, block_dim, 4 * num_topics * sizeof(float), stream1>>>(
      d_cols,
      d_indptr,
      d_vali,
      d_counts,
      ...
      EstepKernel<<<block_cnt, block_dim, 4 * num_topics * sizeof(float), stream2>>>(
        d_cols,
        d_indptr,
        d_vali,
        d_counts,
      ...
      cudaEventRecord(event1[i], stream2);
  }

  cudaDeviceSynchronize();
  auto end = std::chrono::steady_clock::now();
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda lda-cuda main.cu,0);
  ...
  cudaEvent_t event2[(repeat)], event3[(repeat)];  // NUM = number of (A,B) pairs
  for (int i = 0; i < (repeat); ++i) {
      cudaEventCreate(&event2[i]);
      cudaEventCreate(&event3[i]);
  }

  for (i = 0; i < repeat; i++) {
    if(i >= 2)
    {
      cudaStreamWaitEvent(stream1, event3[i - 2], 0);
    }
    EstepKernel<<<block_cnt, block_dim, 4 * num_topics * sizeof(float), stream1>>>(
      d_cols,
      d_indptr,
      ...
      cudaEventRecord(event2[i], stream1);
      cudaStreamWaitEvent(stream2, event2[i], 0);
      ...
      cudaEventRecord(event3[i], stream2);
  }

  cudaDeviceSynchronize();
  end = std::chrono::steady_clock::now();
  time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda lda-cuda main.cu,1);
