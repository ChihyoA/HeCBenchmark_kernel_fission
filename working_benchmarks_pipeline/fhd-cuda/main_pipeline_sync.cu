            ...
            const kdata*__restrict__ k) 
{
  ...
  if (n < samples) {
    ...
    float rfhdn = 0, ifhdn = 0;

    // 나눠진 index 범위 정의
    const int split_start = stream_A ? 0 : (voxels / 2);
    const int split_end   = stream_A ? (voxels / 2) : voxels;

    for (int m = split_start; m < split_end; m++) {
      ...
      atomicAdd(&rfhd[n], rfhdn);
      atomicAdd(&ifhd[n],ifhdn);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  cudaEvent_t event[(nchunks)], event2[(nchunks)];  // NUM = number of (A,B) pairs
  ...
  kdata *kdata_array, *kdata_array_2;
  cudaMalloc((void**)&kdata_array, sizeof(kdata) * CHUNK_S);
  cudaMalloc((void**)&kdata_array_2, sizeof(kdata) * CHUNK_S);
  ...
  for (int i = 0; i < nchunks; i++) {
    ...
    //cudaDeviceSynchronize();
    //cudaMemcpyToSymbol(kdata_array, &h_k[i * CHUNK_S], s);
    //cudaMemcpyToSymbolAsync(kdata_array, &h_k[i * CHUNK_S], s, 0, cudaMemcpyHostToDevice, stream1);
    //cudaMemcpy(kdata_array, &h_k[i * CHUNK_S], s, cudaMemcpyHostToDevice);

    if (i >= 2)
    {
      cudaStreamWaitEvent(stream1, event2[i - 2], 0);
    }
    cudaMemcpyAsync(kdata_array, &h_k[i * CHUNK_S], s, cudaMemcpyHostToDevice, stream1);
    cmpfhd<<<grid, block, 0, stream1>>>(d_rmu + i*CHUNK_S,
                            d_imu + i*CHUNK_S, 
                            d_rfhd, d_ifhd, 
                            d_x, d_y, d_z, 
                            samples, c, true, kdata_array);

    cudaEventRecord(event[i], stream1);
    cudaStreamWaitEvent(stream2, event[i], 0);
    

    cudaMemcpyAsync(kdata_array_2, &h_k[i * CHUNK_S], s, cudaMemcpyHostToDevice, stream2);
    //cudaMemcpy(kdata_array_2, &h_k[i * CHUNK_S], s, cudaMemcpyHostToDevice);
    //cudaMemcpyToSymbol(kdata_array_2, &h_k[i * CHUNK_S], s);
    //cudaMemcpyToSymbolAsync(kdata_array_2, &h_k[i * CHUNK_S], s, 0, cudaMemcpyHostToDevice, stream2);
    cmpfhd<<<grid, block, 0, stream2>>>(d_rmu + i*CHUNK_S,
                              d_imu + i*CHUNK_S, 
                              d_rfhd, d_ifhd, 
                              d_x, d_y, d_z, 
                              samples, c, false, kdata_array_2);

    cudaEventRecord(event2[i], stream2);
    
  }

  cudaDeviceSynchronize();
  auto end = std::chrono::steady_clock::now();
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda fhd-cuda main.cu,0);
