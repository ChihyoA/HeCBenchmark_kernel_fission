__global__ void advCubatureHex3D (
    ...
    bool stream_A,
    dfloat * __restrict r_U_tmp,
    dfloat * __restrict r_V_tmp,
    dfloat * __restrict r_W_tmp,
    dfloat * __restrict r_Ud_tmp,
    dfloat * __restrict r_Vd_tmp,
    dfloat * __restrict r_Wd_tmp
  )
{
  ...
  // 나눠진 index 범위 정의
  const int split_start = stream_A ? 0 : 8;
  const int split_end   = stream_A ? 8 : 16;
  const int global_id = (blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x);
  ...
  else
  {
    for (int k=0; k<16; k++)
    {
      r_U[k] = r_U_tmp[global_id*16 + k];
      r_V[k] = r_V_tmp[global_id*16 + k];
      r_W[k] = r_W_tmp[global_id*16 + k];
    }

    for (int k=0; k<16; ++k)
    {
      r_Ud[k] = r_Ud_tmp[global_id*16 + k];
      r_Vd[k] = r_Vd_tmp[global_id*16 + k];
      r_Wd[k] = r_Wd_tmp[global_id*16 + k];
    }
  }
  ...
  if (stream_A)
  {
    for (int k=0; k<16; k++)
    {
      r_U_tmp[global_id*16 + k] = r_U[k];
      r_V_tmp[global_id*16 + k] = r_V[k];
      r_W_tmp[global_id*16 + k] = r_W[k];
    }
    for (int k=0; k<16; k++)
    {
    r_Ud_tmp[global_id*16 + k] = r_Ud[k];
    r_Vd_tmp[global_id*16 + k] = r_Vd[k];
    r_Wd_tmp[global_id*16 + k] = r_Wd[k];
    }
  }
