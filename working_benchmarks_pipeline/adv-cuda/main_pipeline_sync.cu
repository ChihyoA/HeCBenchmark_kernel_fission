...
int main(int argc, char **argv) {
  ...
  dfloat *r_U_tmp, *r_V_tmp, *r_W_tmp;
  dfloat *r_Ud_tmp, *r_Vd_tmp, *r_Wd_tmp;

  cudaMalloc((void**)&r_U_tmp, 16*256*Nelements*sizeof(dfloat));
  cudaMalloc((void**)&r_V_tmp, 16*256*Nelements*sizeof(dfloat));
  cudaMalloc((void**)&r_W_tmp, 16*256*Nelements*sizeof(dfloat));

  cudaMalloc((void**)&r_Ud_tmp, 16*256*Nelements*sizeof(dfloat));
  cudaMalloc((void**)&r_Vd_tmp, 16*256*Nelements*sizeof(dfloat));
  cudaMalloc((void**)&r_Wd_tmp, 16*256*Nelements*sizeof(dfloat));

  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
      std::cerr << "CUDA Error1: " << cudaGetErrorString(err) << "\n";
  }
  ...
  cudaEvent_t event[(Ntests)], event2[(Ntests)];  // NUM = number of (A,B) pairs
  ...
  for(int test=0;test<Ntests;++test) 
  {
    if (test >= 2)
    {
      cudaStreamWaitEvent(stream1, event2[test - 2], 0);
      cudaEventSynchronize(event2[test-1]); 
    }
    else if (test == 1)
    //cudaStreamWaitEvent(stream1, event2[i-1], 0);
    cudaEventSynchronize(event2[0]); 
    //cudaDeviceSynchronize();
    advCubatureHex3D<<<dim3(Nelements, 1), dim3(16, 16), 0, stream1>>>( 
        Nelements,
        d_vgeo,
        d_cubvgeo,
        d_cubDiffInterpT,
        ...
        offset, 
        d_u,
        d_adv, true, r_U_tmp, r_V_tmp, r_W_tmp, r_Ud_tmp, r_Vd_tmp, r_Wd_tmp);

    cudaError_t err = cudaGetLastError();
        if (err != cudaSuccess) {
            std::cerr << "CUDA Error2: " << cudaGetErrorString(err) << "\n";
        }

    cudaEventRecord(event[test], stream1);
    cudaStreamWaitEvent(stream2, event[test], 0);
    
    //cudaDeviceSynchronize();
    advCubatureHex3D<<<dim3(Nelements, 1), dim3(16, 16), 0, stream2>>>( 
          Nelements,
          d_vgeo,
          d_cubvgeo,
          d_cubDiffInterpT,
          ...
          offset,
          d_u,
          d_adv, false, r_U_tmp, r_V_tmp, r_W_tmp, r_Ud_tmp, r_Vd_tmp, r_Wd_tmp);

    err = cudaGetLastError();
          if (err != cudaSuccess) {
              std::cerr << "CUDA Error3: " << cudaGetErrorString(err) << "\n";
          }

    cudaEventRecord(event2[test], stream2);
    //cudaEventSynchronize(event2[test]);
