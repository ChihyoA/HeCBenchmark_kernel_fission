...
kernel1 (int*__restrict__ d_input_itemsets,
         ...
         const int penalty,
         const bool stream_A, 
         int*__restrict__ input_itemsets_l_tmp,
         int*__restrict__ reference_l_tmp)
{
  ...
  if (stream_A)
  {
    ...
    for( int m = 0 ; m < BLOCK_SIZE ; m++){
      ...
      for (int ty = 0 ; ty < BLOCK_SIZE + 1 ; ty++) {
        input_itemsets_l_tmp[(BLOCK_SIZE + 1) * global_id + ty] = SCORE((ty), (tx));
        if (tx == BLOCK_SIZE - 1)
        {
          input_itemsets_l_tmp[(BLOCK_SIZE + 1) * (global_id + 1) + ty] = SCORE((ty), (tx+1));
        }
      }
      for (int ty = 0 ; ty < BLOCK_SIZE; ty++) {
        reference_l_tmp[(BLOCK_SIZE) * global_id + ty] = SCORE((ty), (tx));
      }
  ...
  else
  {
    ...
    int global_id = (blockIdx.x) * blockDim.x + threadIdx.x;
    for (int ty = 0 ; ty < BLOCK_SIZE + 1 ; ty++) {
      SCORE((ty), (tx)) = input_itemsets_l_tmp[(BLOCK_SIZE + 1) * global_id + ty];
      if (tx == BLOCK_SIZE - 1)
      {
        SCORE((ty), (tx+1)) = input_itemsets_l_tmp[(BLOCK_SIZE + 1) * (global_id + 1) + ty];
      }
    }
    for (int ty = 0 ; ty < BLOCK_SIZE; ty++) {
      SCORE((ty), (tx)) = reference_l_tmp[(BLOCK_SIZE) * global_id + ty];
    }
...
int main(int argc, char **argv){
  ...
  int * input_itemsets_l_tmp;
  int * reference_l_tmp;
  // 여기부터 다시 시작
  //cudaMalloc((void**)&input_itemsets_l_tmp, (BLOCK_SIZE + 1) * (BLOCK_SIZE + 1) * sizeof(int) * (block_width + 1));
  

  cudaEvent_t event[(block_width+1)];  // NUM = number of (A,B) pairs
  ...
  for( int blk = 1 ; blk <= block_width ; blk++){
    global_work = blk;
    kernel1<<<global_work, local_work, 0, stream1>>>(d_input_itemsets, d_reference, offset_r, offset_c, max_cols, blk, penalty, true);
    cudaEventRecord(event[blk], stream1);
    cudaStreamWaitEvent(stream2, event[blk], 0);
    kernel1<<<global_work, local_work, 0, stream2>>>(d_input_itemsets, d_reference, offset_r, offset_c, max_cols, blk, penalty, false);
  ...
  for( int blk = block_width - 1 ; blk >= 1 ; blk--){      
    global_work = blk;
    kernel2<<<global_work, local_work, 0, stream1>>>(d_input_itemsets, d_reference, block_width, offset_r, offset_c, max_cols, blk, penalty, true);
    cudaEventRecord(event[blk], stream1);
    cudaStreamWaitEvent(stream2, event[blk], 0);
    kernel2<<<global_work, local_work, 0, stream2>>>(d_input_itemsets, d_reference, block_width, offset_r, offset_c, max_cols, blk, penalty, false);
    cudaDeviceSynchronize();
  }
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda nw-cuda nw.cu,0);
