                     ...
                     REAL* y)
{
  ...
  for (int i = 0; i < repeat; i++)
  {
    ...
    mv_csr_split_atomic<<<grids, blocks, 0, stream1>>>(num_rows, d_row_indices, d_col_indices, d_values, d_x, d_y, true);

    mv_csr_split_atomic<<<grids, blocks, 0, stream2>>>(num_rows, d_row_indices, d_col_indices, d_values, d_x, d_y, false);
    cudaDeviceSynchronize();
  }
