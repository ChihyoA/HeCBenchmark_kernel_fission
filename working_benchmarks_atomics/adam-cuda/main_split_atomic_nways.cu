  ...
  const float decay)
{
  ...
  // this first for loop only have one iteration, so splitting this doesn't give you performance benefit
...
void adam_split (
  ...
  const float decay,
  const int stream_A,
  const int num_streams)  // 새로 추가된 argument
{
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t totThreads = gridDim.x * blockDim.x;

  // 나눠진 index 범위 정의
  const size_t split_start = stream_A == 0 ? 1 : stream_A * (time_step / num_streams);
  const size_t split_end   = stream_A + 1 == num_streams ? time_step : (stream_A + 1) * (time_step / num_streams);

  for (size_t j = i; j < vector_size; j += totThreads) {
    // if threadid=0 print someting
    //if (i == 0) {
    //  printf("check iteration %lu, vector_size: %lu\n", j, vector_size);
    //}
    for (int t = split_start; t < split_end; t++) {
      T scaled_grad = g[j]/grad_scale;
      m[j] = b1*m[j] + (1.f-b1)*scaled_grad;
      v[j] = b2*v[j] + (1.f-b2)*scaled_grad*scaled_grad;
      float m_corrected = m[j] / (1.f - powf(b1, t));
      ...
      float update = (m_corrected / denom) + (decay * p[j]);
      //p[j] -= (step_size * update);
      atomicAdd(&p[j], -(step_size * update));
    }
  }
}
...
int main(int argc, char* argv[])
{
  ...
  auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda adam-cuda main.cu,0);
  ...
  for (int i = 0; i < repeat; i++) {
    ...
    adam_split<float, float><<<grids, blocks, 0, streams[j]>>> (
      d_p, d_m, d_v, d_g,
      beta1, beta2,
      eps,
      grad_scale,
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda adam-cuda main.cu,0);
  ...
  // 파일 출력 스트림 생성
  std::ofstream p_out("p_output_atomic.txt");
  std::ofstream r_out("r_output_atomic.txt");
