...
void adam_split (
  ...
  adamMode_t mode,
  const float decay,
  const bool stream_A)  // 새로 추가된 argument
{
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t totThreads = gridDim.x * blockDim.x;

  // 나눠진 index 범위 정의
  const size_t split_start = stream_A ? 1 : (time_step / 2);
  const size_t split_end   = stream_A ? (time_step / 2) : time_step;
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < repeat; i++) {
    adam_split<float, float><<<grids, blocks, 0, stream1>>> (
      d_p, d_m, d_v, d_g,
      beta1, beta2,
      eps,
      grad_scale,
      ...
      adam_split<float, float><<<grids, blocks, 0, stream2>>> (
        d_p, d_m, d_v, d_g,
        beta1, beta2,
        eps,
        grad_scale,
