...
__global__ void cenergy(const int numatoms, const float gridspacing, 
                        float *energygrid, const float4 *atominfo, bool stream_A) 
{
  ...
  // 나눠진 index 범위 정의
  const int split_start = stream_A ? 0 : (numatoms / 2);
  const int split_end   = stream_A ? (numatoms / 2) : numatoms;
...
int main(int argc, char** argv) {
  ...
  for (atomstart=0; atomstart<atomcount; atomstart+=MAXATOMS) {   
    ...
    cenergy<<<Gsz, Bsz, 0, stream1>>>(runatoms, 0.1, doutput, datominfo, true);
    cenergy<<<Gsz, Bsz, 0, stream2>>>(runatoms, 0.1, doutput, datominfo, false);
