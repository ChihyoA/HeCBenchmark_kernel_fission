...
__global__ void cenergy(const int numatoms, const float gridspacing, 
                        float *energygrid, const float4 *atominfo, const int stream_A, const int num_streams) 
{
  ...
  // 나눠진 index 범위 정의
  const int split_start = stream_A * (numatoms / num_streams);
  const int split_end   = stream_A + 1 == num_streams ?  numatoms : (stream_A + 1) * (numatoms / num_streams);


  for (atomid=split_start; atomid<split_end; atomid++) {
  ...
  atomicAdd(&energygrid[outaddr + 0 * BLOCKSIZEX], energyvalx1);
  atomicAdd(&energygrid[outaddr + 1 * BLOCKSIZEX], energyvalx2);
  atomicAdd(&energygrid[outaddr + 2 * BLOCKSIZEX], energyvalx3);
  atomicAdd(&energygrid[outaddr + 3 * BLOCKSIZEX], energyvalx4);
  atomicAdd(&energygrid[outaddr + 4 * BLOCKSIZEX], energyvalx5);
  atomicAdd(&energygrid[outaddr + 5 * BLOCKSIZEX], energyvalx6);
  atomicAdd(&energygrid[outaddr + 6 * BLOCKSIZEX], energyvalx7);
  atomicAdd(&energygrid[outaddr + 7 * BLOCKSIZEX], energyvalx8);
}
...
int main(int argc, char** argv) {
  ...
  for (atomstart=0; atomstart<atomcount; atomstart+=MAXATOMS) {   
    ...
    for (int i = 0; i < n_way_split; ++i) {
      cenergy<<<Gsz, Bsz, 0, streams[i]>>>(runatoms, 0.1, doutput, datominfo, i, n_way_split);
    }
