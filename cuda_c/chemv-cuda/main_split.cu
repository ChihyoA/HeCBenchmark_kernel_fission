               ...
               struct ComplexFloat Y[Y_SIZE])
{
  ...
  for (int n = 0; n < REPEAT; n++)
  {
    kernel1_split <<< k1_dimGrid, k1_dimBlock, 0, stream1 >>> (dev_AT, dev_X, dev_Y, alpha_im, alpha_re, true);
    kernel1_split <<< k1_dimGrid, k1_dimBlock, 0, stream2 >>> (dev_AT, dev_X, dev_Y, alpha_im, alpha_re, false);
  }
