...
__global__ void EstepKernel(
  ...
  const bool stream_A,
  float*__restrict__ vali_phi_sum_tmp)
{  
  // storage for block
  ...
  for (int i = blockIdx.x; i < num_indptr; i += gridDim.x) {
    ...
    // 나눠진 index 범위 정의
    const int split_start = stream_A ? 0 : (num_iters / 2);
    const int split_end   = stream_A ? (num_iters / 2) : num_iters;
    ...
    {
      for (int j = threadIdx.x; j < num_topics; j += blockDim.x)
        _vali_phi_sum[j] = vali_phi_sum_tmp[blockIdx.x*num_topics+j];
    }
    

    // iterate E step
    for (int j = split_start; j < split_end; ++j) {
      ...
      for (int k = beg; k < end; ++k) {
        ...
        if (j + 1 == num_iters) {
          ...
          if (threadIdx.x == 0) {
            if (_vali) 
              atomicAdd(&vali_losses[blockIdx.x], _loss);
            else
              atomicAdd(&train_losses[blockIdx.x], _loss);
          }
          __syncthreads();
    ...
    {
      for (int j = threadIdx.x; j < num_topics; j += blockDim.x)
        vali_phi_sum_tmp[blockIdx.x*num_topics+j] = _vali_phi_sum[j];
    }
    else
    {
