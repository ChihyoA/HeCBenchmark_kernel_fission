...
__global__ void binomialOptionsKernel_split(const __TOptionData *__restrict d_OptionData,
  real *__restrict d_CallValue,
  const int totalOptions,
  const bool stream_A)
{
const int opt_id = blockIdx.x;

// Subkernel 분기
if ((stream_A && opt_id >= totalOptions / 2) || (!stream_A && opt_id < totalOptions / 2)) {
return; // 해당 stream에서 처리하지 않는 block이면 종료
}

__shared__ real call_exchange[THREADBLOCK_SIZE + 1];

const int tid = threadIdx.x;
const real S = d_OptionData[opt_id].S;
const real X = d_OptionData[opt_id].X;
const real vDt = d_OptionData[opt_id].vDt;
const real puByDf = d_OptionData[opt_id].puByDf;
const real pdByDf = d_OptionData[opt_id].pdByDf;
