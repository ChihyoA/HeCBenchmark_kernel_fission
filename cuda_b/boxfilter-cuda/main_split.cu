...
__global__ void col_kernel_split (
  const uint* __restrict__ uiSource,
        uint* __restrict__ uiDest,
  const unsigned int uiWidth, 
  const unsigned int uiHeight, 
...
// Stream-based 분기 처리
if ((stream_A && globalPosX >= uiWidth / 2) || (!stream_A && globalPosX < uiWidth / 2)) {
  return;
}
...
f4Sum = top_color * make_float4((float)iRadius, (float)iRadius, (float)iRadius, (float)iRadius); 
for (int y = 0; y < iRadius + 1; y++) 
{
  f4Sum += rgbaUintToFloat4(uiInputImage[y * uiWidth]);
}
    ...
    const int iCycles )
{
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda boxfilter-cuda main.cu,0);
