    ...
    const int iCycles )
{
  ...
  for (int i = 0; i < iCycles; i++) {
    ...
    // print every function argument and kernel launch parameters
    printf("Grid: %d, Block: %d\n", row_grid.x, row_block.x);
    printf("uiWidth: %d, uiHeight: %d, iRadius: %d, fScale: %f\n", uiWidth, uiHeight, iRadius, fScale);
