...
template <typename T>
__global__ void vol2col_kernel_split(
...
{
  // 나눠진 index 범위 정의
  const int64_t split_start = stream_A ? 0 : (range / 2);
  const int64_t split_end   = stream_A ? (range / 2) : range;
  printf("split_start = %ld, split_end = %ld, range: %ld\n", split_start, split_end, range);
  printf("stride: %ld\n", blockDim.x * gridDim.x);

  for (int64_t n = blockDim.x * blockIdx.x + threadIdx.x + split_start;
               n < split_end; n += blockDim.x * gridDim.x) {
    ...
    const int dilation_w)
{
  ...
  for (int i = 0; i < repeat; i++) {
    vol2col_kernel_split<T><<<blocksPerGrid, threadsPerBlock, 0, stream1>>>(
      n,
      d_data_vol,
      depth, height, width,
      ...
      d_data_col,
      true);
    vol2col_kernel_split<T><<<blocksPerGrid, threadsPerBlock, 0, stream2>>>(
        n,
        d_data_vol,
        depth, height, width,
        ksize_t, ksize_h, ksize_w,
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda vol2col-cuda main.cu,0);
  ...
  time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda vol2col-cuda main.cu,1);
