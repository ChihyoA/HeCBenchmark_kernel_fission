...
__global__ void  
qrng (float* output, const unsigned int* table, const unsigned int seed, const unsigned int N, const bool stream_A)
{
...
__global__ void  
icnd (float* output, const unsigned int pathN, const unsigned int distance, const bool stream_A)
{
...
int main(int argc, const char **argv)
{
  ...
  {
    qrng<<<grid, block, 0, stream1>>> (d_Output, d_Table, seed, N, true);
    qrng<<<grid, block, 0, stream2>>> (d_Output, d_Table, seed, N, false);
  }
  ...
  for (int i = 0; i < repeat; i++)
  {
    icnd<<<grid2, block2, 0, stream1>>>(d_Output, pathN, distance, true);
    icnd<<<grid2, block2, 0, stream2>>>(d_Output, pathN, distance, false);
  }
