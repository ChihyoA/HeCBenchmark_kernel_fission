...
void gramschmidtCuda(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(R, NJ, NJ, nj, nj), DATA_TYPE POLYBENCH_2D(Q, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(A_outputFromGpu, NI, NJ, ni, nj))
{
    ...
    // Begin CUDA graph capture on the stream
    cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal);

    for (int k = 0; k < _PB_NJ; k++)
    {
        // Launch Kernel 1 using the captured stream
        gramschmidt_kernel1<<<gridKernel1, block, 0, stream>>>(ni, nj, A_gpu, R_gpu, Q_gpu, k);

        // Launch Kernel 2 using the captured stream
        gramschmidt_kernel2<<<gridKernel2, block, 0, stream>>>(ni, nj, A_gpu, R_gpu, Q_gpu, k);

        // Launch Kernel 3 using the captured stream
        gramschmidt_kernel3<<<gridKernel3, block, 0, stream>>>(ni, nj, A_gpu, R_gpu, Q_gpu, k);
    }

    // End CUDA graph capture and retrieve the graph
    cudaGraph_t graph;
