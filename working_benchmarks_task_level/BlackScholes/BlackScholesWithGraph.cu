...
////////////////////////////////////////////////////////////////////////////////
#include "BlackScholes_kernel_split.cuh"
...
int main(int argc, char **argv)
{
    ...
    *d_OptionYears;
    ...
    cudaStream_t stream1, stream2, stream3, graphStream;
    ...
    cudaEvent_t event_x, event_y, event_combine;

    cudaEventCreate(&event_x);
    cudaEventCreate(&event_y);
    cudaEventCreate(&event_combine);

    int threadsPerBlock = 128;
    int blocksPerGrid = DIV_UP(OPT_N, threadsPerBlock);
    ...
    // Dummy kernel to establish dependency
    ...
    cudaStreamWaitEvent(stream3, dummyEvent);
    
    for (i = 0; i < NUM_ITERATIONS; i++)
    {
	// Launch first kernel on stream1
        BlackScholesGPU_X<<<DIV_UP((OPT_N/2), 128), 128, 0, stream1>>>(
            callResultX,          // float*
            putResultX,           // float*
            (const float2*)d_StockPrice,    // cast to const float2*
            (const float2*)d_OptionStrike,  // cast to const float2*
            (const float2*)d_OptionYears,   // cast to const float2*
            RISKFREE, 
            VOLATILITY, 
            OPT_N
        );
        ...
        BlackScholesGPU_Y<<<DIV_UP((OPT_N/2), 128), 128, 0, stream2>>>(
            callResultY,          // float*
            putResultY,           // float*
            (const float2*)d_StockPrice,    // cast to const float2*
            (const float2*)d_OptionStrike,  // cast to const float2*
        ...
        cudaStreamWaitEvent(stream3, event_x);
	cudaStreamWaitEvent(stream3, event_y);

        // Combine the results
        CombineResults<<<DIV_UP((OPT_N/2), 128), 128, 0, stream3>>>(
            (float2*)d_CallResult,    // output float2*
            (float2*)d_PutResult,     // output float2*
            callResultX,              // input float*
            callResultY,              // input float*
            putResultX,               // input float*
            putResultY,               // input float*
            OPT_N
        );
	cudaEventRecord(event_combine, stream3);

        getLastCudaError("BlackScholesGPU() execution failed\n");
    }

    cudaStreamWaitEvent(graphStream, event_combine);
