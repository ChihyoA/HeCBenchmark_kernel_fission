...
__global__ void calculateForceX(double phi[][DATAYSIZE][DATAXSIZE],
    ...
    if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
        (iz < (DATAZSIZE-1)) && (ix > 0) && (iy > 0) && (iz > 0)) {
        
        //double phix = GradientX(phi,dx,dy,dz,ix,iy,iz);
        double phix = (phi[ix+1][iy][iz] - phi[ix-1][iy][iz]) / (2.0*dx);
        //double phiy = GradientY(phi,dx,dy,dz,ix,iy,iz);
        double phiy = (phi[ix][iy+1][iz] - phi[ix][iy-1][iz]) / (2.0*dy);
        //double phiz = GradientZ(phi,dx,dy,dz,ix,iy,iz);
        double phiz = (phi[ix][iy][iz+1] - phi[ix][iy][iz-1]) / (2.0*dz);

        double d_func = 0.0;
        if (phix != 0.0 || phiy != 0.0 || phiz != 0.0){
          d_func = (((phix*phix*phix*(SQ(phiy)+SQ(phiz)))-(phix*(SQ(phiy)*SQ(phiy)+SQ(phiz)*SQ(phiz)))) /
                  ((SQ(phix)+SQ(phiy)+SQ(phiz))*(SQ(phix)+SQ(phiy)+SQ(phiz))));
        }

        double a_n = 0.0;;
        if (phix != 0.0 || phiy != 0.0 || phiz != 0.0){
          a_n = ((1.0 - 3.0 * epsilon) * (1.0 + (((4.0 * epsilon) / (1.0-3.0*epsilon))*
                ((SQ(phix)*SQ(phix)+SQ(phiy)*SQ(phiy)+SQ(phiz)*SQ(phiz)) /
                ((SQ(phix)+SQ(phiy)+SQ(phiz))*(SQ(phix)+SQ(phiy)+SQ(phiz)))))));
        }
        else {
          a_n = (1.0-((5.0/3.0)*epsilon));
        }
        ...
        Fx[ix][iy][iz] = w2 * phix + sqGphi * w * c * d_func;
    } else {
        Fx[ix][iy][iz] = 0.0;
    }
}

__global__ void calculateForceY(double phi[][DATAYSIZE][DATAXSIZE],
    ...
    if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
        ...
        double d_func = 0.0;
        if (phiy != 0.0 || phiz != 0.0 || phix != 0.0){
          d_func = (((phiy*phiy*phiy*(SQ(phiz)+SQ(phix)))-(phiy*(SQ(phiz)*SQ(phiz)+SQ(phix)*SQ(phix)))) /
                  ((SQ(phiy)+SQ(phiz)+SQ(phix))*(SQ(phiy)+SQ(phiz)+SQ(phix))));
        }

        Fy[ix][iy][iz] = w2 * phiy + sqGphi * w * c * d_func;
    } else {
        Fy[ix][iy][iz] = 0.0;
    }
}

__global__ void calculateForceZ(double phi[][DATAYSIZE][DATAXSIZE],
    ...
    if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
        ...
        double d_func = 0.0;
        if (phiz != 0.0 || phix != 0.0 ||  phiy != 0.0){
          d_func = (((phiz*phiz*phiz*(SQ(phix)+SQ(phiy)))-(phiz*(SQ(phix)*SQ(phix)+SQ(phiy)*SQ(phiy)))) /
                  ((SQ(phiz)+SQ(phix)+SQ(phiy))*(SQ(phiz)+SQ(phix)+SQ(phiy))));
        }

        Fz[ix][iy][iz] = w2 * phiz + sqGphi * w * c * d_func;
    } else {
        Fz[ix][iy][iz] = 0.0;
    }
}

__global__ void allenCahnEvolution(double phi[][DATAYSIZE][DATAXSIZE],
                           double tau[][DATAYSIZE][DATAXSIZE],
                           double dx, double dy, double dz,
                           double epsilon, double tau0) {
    unsigned iz = blockIdx.x*blockDim.x + threadIdx.x;
    unsigned iy = blockIdx.y*blockDim.y + threadIdx.y;
    unsigned ix = blockIdx.z*blockDim.z + threadIdx.z;
    ...
    if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
        ...
        tau[ix][iy][iz] = tau0 * SQ(a_n);
    }
}

__global__ void allenCahnEvolution(double phinew[][DATAYSIZE][DATAXSIZE],
    ...
    if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
        ...
        double phix = (phiold[ix+1][iy][iz] - phiold[ix-1][iy][iz]) / (2.0*dx);
        //double phiy = GradientY(phiold,dx,dy,dz,ix,iy,iz);
        double phiy = (phiold[ix][iy+1][iz] - phiold[ix][iy-1][iz]) / (2.0*dy);
        //double phiz = GradientZ(phiold,dx,dy,dz,ix,iy,iz);
        double phiz = (phiold[ix][iy][iz+1] - phiold[ix][iy][iz-1]) / (2.0*dz);

        double gradient_x = (Fx[ix+1][iy][iz] - Fx[ix-1][iy][iz]) / (2.0*dx);
        double gradient_y = (Fy[ix][iy+1][iz] - Fy[ix][iy-1][iz]) / (2.0*dy);
        double gradient_z = (Fz[ix][iy][iz+1] - Fz[ix][iy][iz-1]) / (2.0*dz);

        double divergence = gradient_x + 
                            gradient_y +
                            gradient_z;
        ...
        double phi = phiold[ix][iy][iz];
        double u = uold[ix][iy][iz];
        double d_phi = (-phi*(1.0-phi*phi)+lambda*u*(1.0-phi*phi)*(1.0-phi*phi));
               ...
               double dt, double dx, double dy, double dz)
{
  ...
  if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
    ...
    double divergence = GradientX(Fx,dx,dy,dz,ix,iy,iz) + 
                        GradientY(Fy,dx,dy,dz,ix,iy,iz) +
                        GradientZ(Fz,dx,dy,dz,ix,iy,iz);
                     ...
                     double D, double dt, double dx, double dy, double dz)
{
  ...
  if ((ix < (DATAXSIZE-1)) && (iy < (DATAYSIZE-1)) && 
      ...
      (iy > (0)) && (iz > (0)))
  {
    double phixx = (uold[ix+1][iy][iz] + uold[ix-1][iy][iz] - 2.0 * uold[ix][iy][iz]) / SQ(dx);
    double phiyy = (uold[ix][iy+1][iz] + uold[ix][iy-1][iz] - 2.0 * uold[ix][iy][iz]) / SQ(dy);
    double phizz = (uold[ix][iy][iz+1] + uold[ix][iy][iz-1] - 2.0 * uold[ix][iy][iz]) / SQ(dz);
...
int main(int argc, char *argv[])
{
  ...
  while (t <= num_steps) {
    ...
    calculateForceX<<<grid, block>>>(d_phiold, d_Fx, dx, dy, dz, epsilon, W0);
    calculateForceY<<<grid, block>>>(d_phiold, d_Fy, dx, dy, dz, epsilon, W0);
    calculateForceZ<<<grid, block>>>(d_phiold, d_Fz, dx, dy, dz, epsilon, W0);
    
    allenCahnEvolution<<<grid, block>>>(d_phinew, d_phiold, d_uold,
                                      d_Fx, d_Fy, d_Fz,
