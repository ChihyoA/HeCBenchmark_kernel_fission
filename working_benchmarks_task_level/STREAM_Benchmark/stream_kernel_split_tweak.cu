...
template <typename T>
__global__ void stream_kernel_split_tweak(T const * __restrict__ const a, T * __restrict__ const b,  T * __restrict__ const c, T scale,  int len, int mode)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
...
int main(int argc, char** argv)
{
    ...
    for (k=0; k<NTIMES; k++)
    {
        ...
        stream_kernel_split_tweak<real><<<dimGrid,dimBlock, 0, stream0>>>(d_a, d_b, d_c, scalar, N, 0);
        stream_kernel_split_tweak<real><<<dimGrid,dimBlock, 0, stream1>>>(d_a, d_b, d_c, scalar, N, 1);
        stream_kernel_split_tweak<real><<<dimGrid,dimBlock, 0, stream2>>>(d_a, d_b, d_c, scalar, N, 2);
        stream_kernel_split_tweak<real><<<dimGrid,dimBlock, 0, stream3>>>(d_a, d_b, d_c, scalar, N, 3);
    ...
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(end_cpu - start_cpu).count();
    std::cout << "CPU Total time: " << us << " ms" << std::endl;
