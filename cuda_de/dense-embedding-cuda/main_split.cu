    ...
    const bool stream_A)
{
  ...
  const int split_start = stream_A ? 0 : (embedding_dim / 2);
  const int split_end = stream_A ? (embedding_dim / 2) : embedding_dim;
...
int main(int argc, char* argv[])
{
  ...
  for (int ncols = 64; ncols <= 2048; ncols = ncols * 2) {
    ...
    {
      dense_esuhm_split<<<grid, block, 0, stream1>>>(d_input, d_dense, d_output, ncols, d_input_offset, true);
      dense_esuhm_split<<<grid, block, 0, stream2>>>(d_input, d_dense, d_output, ncols, d_input_offset, false);
    }
    ...
    for (int i = 0; i < repeat; i++) 
    {
      dense_esuhm2_split<<<grid, block, 0, stream1>>>(d_input, d_dense, d_output, ncols, d_input_offset, true);
      dense_esuhm2_split<<<grid, block, 0, stream2>>>(d_input, d_dense, d_output, ncols, d_input_offset, false);
    }
