  ...
  const bool stream_A)  // 새로 추가된 argument
{
  ...
  // 나눠진 index 범위 정의
  const size_t split_start = stream_A ? 0 : (vector_size / 2);
  const size_t split_end   = stream_A ? (vector_size / 2) : vector_size;

  for (size_t j = split_start + i; j < split_end; j += totThreads) {
