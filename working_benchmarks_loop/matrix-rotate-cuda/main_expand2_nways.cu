...
__global__ void rotate_matrix_parallel (float *matrix, const int n, const int num_streams) {\
  int original_griddim = gridDim.x / num_streams;
  int blockidx_x = blockIdx.x % original_griddim;
  int block_chunk_idx = blockIdx.x / original_griddim;

  int layer = blockidx_x * blockDim.x + threadIdx.x;
  if (layer < n/2) {
    int first = layer;
    int last = n - 1 - layer;

    int split_start = block_chunk_idx * ((last-first) / num_streams ) + first;
    int split_end = block_chunk_idx + 1 == num_streams ? last : first + (block_chunk_idx + 1)* (((last-first)) / num_streams);
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < repeat; i++) {
    rotate_matrix_parallel<<<(n/2+255)/256*n_way_split, 256>>>(d_parallel_res, n, n_way_split);
  }
