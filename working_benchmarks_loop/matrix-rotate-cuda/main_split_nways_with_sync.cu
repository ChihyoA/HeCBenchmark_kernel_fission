...
__global__ void rotate_matrix_parallel_pipeline (float *matrix, const int n, const int stream_A, const int num_streams) {
  ...
  if (layer < n/2) {
    ...
    const int split_start = stream_A * (((last-first)) / num_streams);
    const int split_end = stream_A + 1 == num_streams ? last : first + (stream_A + 1)* (((last-first)) / num_streams);
    
    //printf("split_start: %d, split_end: %d\n", split_start, split_end);

    for(int i = split_start + first; i < split_end; ++i) {
      int offset = i - first;
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < repeat; i++) {
    ...
    rotate_matrix_parallel_pipeline<<<(n/2+255)/256, 256, 0, streams[j]>>>(d_parallel_res, n, j, n_way_split);
    }
    for (int j = 0; j < n_way_split; ++j) {
