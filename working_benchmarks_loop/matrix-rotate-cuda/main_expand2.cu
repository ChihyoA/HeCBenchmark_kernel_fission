...
__global__ void rotate_matrix_parallel (float *matrix, const int n) {\
  ...
  if (layer < n/2) {
    ...
    int split_start = blockIdx.x >= gridDim.x / 2 ? first : (((last-first) / 2 ) + first);
    int split_end = blockIdx.x >= gridDim.x / 2 ? (((last-first) / 2 ) + first): last;;
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < repeat; i++) {
    rotate_matrix_parallel<<<(n/2+255)/128, 256>>>(d_parallel_res, n);
  }
