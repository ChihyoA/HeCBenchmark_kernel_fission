  ...
  const bool stream_A)
{
  ...
  // 나눠진 index 범위 정의
  const int split_start = stream_A ? 0 : (data_size / 2);
  const int split_end   = stream_A ? (data_size / 2) : data_size;
  //printf("data_size %d, split_start %d, split_end %d\n", data_size, split_start, split_end);
  //printf("idx: %d, stride: %d\n", idx, NUM_THREADS*NUM_BLOCKS);

  for(int i = idx+split_start; i < split_end; i += NUM_THREADS*NUM_BLOCKS)
  {
...
MY_START_CLOCK(cuda spm-cuda main.cu,0);
  ...
  {
    spm_split<<<NUM_BLOCKS,NUM_THREADS, 0, stream1>>>(M_d, vol_size, g_d, f_d, g_vol, f_vol,
                                    ivf_d,ivg_d,data_threshold_d, true);
    spm_split<<<NUM_BLOCKS,NUM_THREADS, 0, stream2>>>(M_d, vol_size, g_d, f_d, g_vol, f_vol,
                                      ivf_d,ivg_d,data_threshold_d, false);
                        cudaDeviceSynchronize();
  }
...
MY_STOP_CLOCK(cuda spm-cuda main.cu,0);
