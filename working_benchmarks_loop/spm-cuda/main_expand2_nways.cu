  ...
  const int num_streams)
{
  ...
  const int block_idx = (blockIdx.x % NUM_BLOCKS);
  const int idx = block_idx * NUM_THREADS + threadIdx.x;
  const int block_chunk_idx = blockIdx.x / NUM_BLOCKS;

  int x_datasize=(dg.x-2);
  int y_datasize=(dg.y-2);

  const int split_start = block_chunk_idx * ((data_size) / num_streams);
  const int split_end = block_chunk_idx + 1 == num_streams ? data_size : (block_chunk_idx + 1)* ((data_size) / num_streams);
...
MY_START_CLOCK(cuda spm-cuda main.cu,0);
  ...
  for (int i = 0; i < repeat; i++)
    spm<<<NUM_BLOCKS*n_way_split,NUM_THREADS>>>(M_d, vol_size, g_d, f_d, g_vol, f_vol,
                                    ivf_d,ivg_d,data_threshold_d, n_way_split);
