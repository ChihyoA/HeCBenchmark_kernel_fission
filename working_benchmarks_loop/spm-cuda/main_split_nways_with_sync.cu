  ...
  const int num_streams)
{
  ...
  const int split_start = stream_A * ((data_size) / num_streams);
  const int split_end = stream_A + 1 == num_streams ? data_size : (stream_A + 1)* ((data_size) / num_streams);
...
MY_START_CLOCK(cuda spm-cuda main.cu,0);
  ...
  for (int i = 0; i < repeat; i++)
  {
    ...
    spm_split<<<NUM_BLOCKS,NUM_THREADS, 0, streams[j]>>>(M_d, vol_size, g_d, f_d, g_vol, f_vol,
                                    ivf_d,ivg_d,data_threshold_d, j, n_way_split);
    }
    for (int j = 0; j < n_way_split; ++j) {
