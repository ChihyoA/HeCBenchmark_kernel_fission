  ...
  bool *__restrict__ data_threshold_d)
{
  ...
  const int idx = blockIdx.x < NUM_BLOCKS ? blockIdx.x * NUM_THREADS + threadIdx.x : (blockIdx.x - NUM_BLOCKS) * NUM_THREADS + threadIdx.x;

  int x_datasize=(dg.x-2);
  int y_datasize=(dg.y-2);

  const int split_start = blockIdx.x < NUM_BLOCKS ? 0 : (data_size / 2);
  const int split_end   = blockIdx.x < NUM_BLOCKS ? (data_size / 2) : data_size;
...
MY_START_CLOCK(cuda spm-cuda main.cu,0);
  ...
  for (int i = 0; i < repeat; i++)
    spm<<<NUM_BLOCKS*2,NUM_THREADS>>>(M_d, vol_size, g_d, f_d, g_vol, f_vol,
