  ...
  bool *__restrict__ data_threshold_d)
{
  ...
  int i = blockIdx.y * NUM_THREADS * NUM_BLOCKS + idx;
  if (i < data_size) // ensure we do not go out of bounds
  {
...
MY_START_CLOCK(cuda spm-cuda main.cu,0);
  ...
  dim3 grids(NUM_BLOCKS, (vol_size + NUM_BLOCKS * NUM_THREADS - 1) / (NUM_BLOCKS * NUM_THREADS));
  for (int i = 0; i < repeat; i++)
    spm<<<grids,NUM_THREADS>>>(M_d, vol_size, g_d, f_d, g_vol, f_vol,
