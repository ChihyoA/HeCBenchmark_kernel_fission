  ...
  bool *__restrict__ data_threshold_d)
{
  ...
  const int idx = (blockIdx.x / 2) * NUM_THREADS + threadIdx.x;

  int x_datasize=(dg.x-2);
  int y_datasize=(dg.y-2);

  int iter_start = (blockIdx.x % 2 == 0) ? 0 : NUM_THREADS*NUM_BLOCKS;

  for(int i = idx + iter_start; i < data_size; i += NUM_THREADS*NUM_BLOCKS*2)
  {
