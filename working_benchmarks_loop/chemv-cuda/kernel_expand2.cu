...
{
    int b0 = blockIdx.x >= 12 ? blockIdx.x-12 : blockIdx.x;
    int t0 = threadIdx.x;
    ...
    const int split_start = blockIdx.x >= 12 ? 5888 * b0 : 5888 * b0 + loop_region;
    const int split_end   = blockIdx.x >= 12 ? 5888 * b0 + loop_region : 5888 * b0 + loop_region * 2;  
