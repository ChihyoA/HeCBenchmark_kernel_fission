...
__global__ void kernel1(struct ComplexFloat *AT, struct ComplexFloat *X, struct ComplexFloat *Y, float alpha_im, float alpha_re, const int num_streams)
{
    int b0 = blockIdx.x % 12;
    int block_chunk_idx = blockIdx.x / 12;
    ...
    const int loop_region = (ppcg_min(67712, 5856 * b0 + 6016) - 5888 * b0) / num_streams;
    const int split_start = 5888 * b0 + block_chunk_idx * loop_region;
    const int split_end   = block_chunk_idx+1 == num_streams ? ppcg_min(67712, 5856 * b0 + 6016) : 5888 * b0 + (block_chunk_idx + 1) * loop_region;  

    for (int c1 = split_start; c1 <= split_end; c1 += 32) {

    //for (int c1 = 5888 * b0 + 32 * threadIdx.x % 2; c1 <= ppcg_min(67712, 5856 * b0 + 6016); c1 += 64) {
