...
__global__ void kernel1(struct ComplexFloat *AT, struct ComplexFloat *X, struct ComplexFloat *Y, float alpha_im, float alpha_re)
{
    ...
    // Loop iteration number is not constant, flatting the loop will become very inefficient 
    // for (int c1 = 5888 * b0; c1 <= ppcg_min(67712, 5856 * b0 + 6016); c1 += 32) {
    int c1 = 5888 * b0 + 32 * blockIdx.y;
    if (c1 <= ppcg_min(67712, 5856 * b0 + 6016)) { // Ensure c1 is within the bounds of the loop
      ...
      // Not sure why we have syncthreads here
      __syncthreads();
    }
}
