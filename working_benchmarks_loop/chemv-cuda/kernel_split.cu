...
__global__ void kernel1_split(struct ComplexFloat *AT, struct ComplexFloat *X, struct ComplexFloat *Y, float alpha_im, float alpha_re, bool stream_A)
{
    int b0 = blockIdx.x; //stream_A ? blockIdx.x : blockIdx.x + gridDim.x;
    int t0 = threadIdx.x;
    ...
    const int loop_region = (ppcg_min(67712, 5856 * b0 + 6016) - 5888 * b0) / 2;
    const int split_start = stream_A ? 5888 * b0 : 5888 * b0 + loop_region;
    const int split_end   = stream_A ? 5888 * b0 + loop_region : 5888 * b0 + loop_region * 2;  
