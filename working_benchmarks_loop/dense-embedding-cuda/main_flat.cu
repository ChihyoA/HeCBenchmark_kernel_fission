    ...
    const int* offset)
{
  ...
  int idx = tid + blockIdx.y * grain_size;
    const T dense_elem = dense[batch_idx * embedding_dim + idx];
    for (int nested_idx = idx; nested_idx < range; nested_idx += embedding_dim) {
...
//  for (int idx = threadIdx.x; idx < embedding_dim; idx += blockDim.x) {
  int idx = threadIdx.x + blockIdx.y * blockDim.x; // this allows to use multiple blocks in y dim
    const T dense_elem = dense[batch_idx * embedding_dim + idx];
    for (int nested_idx = idx; nested_idx < range; nested_idx += embedding_dim) {
...
int main(int argc, char* argv[])
{
  ...
  for (int ncols = 64; ncols <= 2048; ncols = ncols * 2) {
    ...
    {
      dim3 grid (batch_size, max(ncols/256, 1));
      //printf("Grid dimensions: (%d, %d), ncols=%d\n", grid.x, grid.y, ncols);
      dim3 block (256);
