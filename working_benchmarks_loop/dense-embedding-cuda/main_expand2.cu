...
__global__ void dense_esuhm(
    ...
    const int* offset)
{
  const int batch_idx  = blockIdx.x >= gridDim.x / 2 ? blockIdx.x - gridDim.x / 2 : blockIdx.x; // each batch is handled by a block
  const int grain_size = blockDim.x;
  const int tid = threadIdx.x;
  const int range = offset[batch_idx + 1] - offset[batch_idx];

  const int split_start = blockIdx.x >= gridDim.x / 2 ? 0 : (embedding_dim / 2);
  const int split_end = blockIdx.x >= gridDim.x / 2 ? (embedding_dim / 2) : embedding_dim;
