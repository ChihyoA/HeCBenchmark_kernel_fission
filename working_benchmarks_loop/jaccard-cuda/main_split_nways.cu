          ...
          const int n_way_split) 
{
  ...
  for (int row=threadIdx.z+blockIdx.z*blockDim.z; row<n; row+=gridDim.z*blockDim.z) { 
    ...
    const int interval = (csrPtr[row+1] - csrPtr[row]) / (gridDim.y*blockDim.y) / n_way_split;

    const int split_start = csrPtr[row]+threadIdx.y+blockIdx.y*blockDim.y + stream_A * interval * gridDim.y*blockDim.y;
    const int split_end   = stream_A + 1 == n_way_split ? csrPtr[row+1] : csrPtr[row] + threadIdx.y+blockIdx.y*blockDim.y + (stream_A + 1) * interval * gridDim.y*blockDim.y;
    ...
    int* csr_ptr, int* csr_ind, T* csr_val)
{
  ...
  for (int i = 0; i < iteration; i++) {
    ...
    for (int i = 0; i < n_way_split; ++i) {
    jaccard_is<weighted,T><<<nblocks,nthreads, 0, streams[i]>>>(n, e, d_csrPtr,
        d_csrInd, d_weight_j, d_work, d_weight_i, d_weight_s, i, n_way_split);
    }
