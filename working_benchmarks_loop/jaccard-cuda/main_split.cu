          ...
          const bool stream_A) 
{
  ...
  // print streamA
  //printf("stream_A: %d", stream_A);

  for (int row=threadIdx.z+blockIdx.z*blockDim.z; row<n; row+=gridDim.z*blockDim.z) { 

    const int interval = (csrPtr[row+1] - csrPtr[row]) / (gridDim.y*blockDim.y) / 2;

    const int split_start = stream_A ? csrPtr[row]+threadIdx.y+blockIdx.y*blockDim.y : csrPtr[row]+threadIdx.y+blockIdx.y*blockDim.y + interval * gridDim.y*blockDim.y;
    const int split_end   = stream_A ? csrPtr[row]+threadIdx.y+blockIdx.y*blockDim.y + interval * gridDim.y*blockDim.y : csrPtr[row+1];

    for (int j=split_start;
             j<split_end; j+=gridDim.y*blockDim.y) { 
      int col = csrInd[j];
    ...
    int* csr_ptr, int* csr_ind, T* csr_val)
{
  ...
  for (int i = 0; i < iteration; i++) {
    ...
    //printf("n: %d, e: %d, nblocks.z: %d\n", n, e, nblocks.z);



    jaccard_is<weighted,T><<<nblocks,nthreads, 0, stream1>>>(n, e, d_csrPtr,
        d_csrInd, d_weight_j, d_work, d_weight_i, d_weight_s, true);

    jaccard_is<weighted,T><<<nblocks,nthreads, 0, stream2>>>(n, e, d_csrPtr,
          d_csrInd, d_weight_j, d_work, d_weight_i, d_weight_s, false);
    //printf("stream1 = %p\n", (void*)stream1);
    //printf("stream2 = %p\n", (void*)stream2);
