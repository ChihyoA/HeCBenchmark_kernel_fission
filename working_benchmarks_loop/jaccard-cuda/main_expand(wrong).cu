...
{
  const int gid_z = (blockIdx.z / 2) * blockDim.z + threadIdx.z;
  const int split_start = blockIdx.z % 2 ? 0 : (gridDim.z / 2 * blockDim.z);

  for (int row=gid_z + split_start; row<n; row+=gridDim.z*blockDim.z) {  
