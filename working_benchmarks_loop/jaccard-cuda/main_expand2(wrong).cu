                 ...
                 T *__restrict__ weight_s) 
{
  ...
  const int batch_idx  = blockIdx.z >= (gridDim.z / 2 )? blockIdx.z - (gridDim.z / 2) : blockIdx.z; // each batch is handled by a block

  const int split_start = blockIdx.z >= (gridDim.z / 2) ? (n / 2): 0;
  const int split_end = blockIdx.z >= (gridDim.z / 2) ? n : (n / 2);
  //printf("number of rows: %d, split_start: %d, split_end: %d\n", n, split_start, split_end);
  //printf("batch_idx: %d, blockIdx.z: %d, gridDim.z: %d\n", batch_idx, blockIdx.z, gridDim.z);

  const int gid_z = threadIdx.z + batch_idx * blockDim.z;

  for (int row=gid_z + split_start; row<split_end; row+=gridDim.z/2*blockDim.z) {  
    ...
    int* csr_ptr, int* csr_ind, T* csr_val)
{
  ...
  for (int i = 0; i < iteration; i++) {
    ...
    nblocks.x  = 1;
    nblocks.y  = 1;
    nblocks.z  = 2*(n + nthreads.z - 1)/nthreads.z; // less than CUDA_MAX_BLOCKS);
    // print kernel dimension
    // printf("Kernel dimension: %d x %d x %d, size: %d\n", nblocks.x, nblocks.y, nblocks.z, e);
