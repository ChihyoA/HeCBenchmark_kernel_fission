...
{
  unsigned int globalID_x   = blockIdx.x < gridDim.x / 2 ? 
  blockIdx.x * blockDim.x + threadIdx.x : (blockIdx.x - gridDim.x/2) * blockDim.x + threadIdx.x;
  unsigned int localID_y    = threadIdx.y;
  unsigned int globalSize_x = gridDim.x * blockDim.x / 2;

  unsigned int iter_start = blockIdx.x < gridDim.x / 2 ? 0 : N / 2;
  unsigned int iter_end = blockIdx.x < gridDim.x / 2 ? N / 2 : N;

  for (unsigned int pos = globalID_x + iter_start; pos < iter_end; pos += globalSize_x) {
...
{
  const unsigned int globalID   = blockIdx.x < gridDim.x / 2 ? blockIdx.x * blockDim.x + threadIdx.x : (blockIdx.x - gridDim.x/2) * blockDim.x + threadIdx.x;
  const unsigned int globalSize = gridDim.x * blockDim.x / 2;

  unsigned int iter_start = blockIdx.x < gridDim.x / 2 ? 0 : pathN / 2;
  unsigned int iter_end = blockIdx.x < gridDim.x / 2 ? pathN / 2 : pathN;

  for(unsigned int pos = globalID + iter_start; pos < iter_end; pos += globalSize){
...
int main(int argc, const char **argv)
{
  ...
  dim3 grid (2 * globalWorkSize[0] / localWorkSize[0], globalWorkSize[1] / localWorkSize[1]);
