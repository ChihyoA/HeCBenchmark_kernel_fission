...
qrng (float* output, const unsigned int* table, const unsigned int seed, const unsigned int N)
{
  ...
  int global_grid_idx = blockIdx.z * globalSize_x;
 
  //for (unsigned int pos = globalID_x ; pos < N; pos += globalSize_x) {
    int pos = globalID_x + global_grid_idx;
    if(pos < N)
    {
...
icnd (float* output, const unsigned int pathN, const unsigned int distance)
{
  ...
  int global_grid_idx = blockIdx.y * globalSize;

  //for(unsigned int pos = globalID; pos < pathN; pos += globalSize){
    int pos = globalID + global_grid_idx;
    if(pos < pathN){
...
int main(int argc, const char **argv)
{
  ...
  size_t z_dim = N;
  //dim3 grid (globalWorkSize[0] / localWorkSize[0], globalWorkSize[1] / localWorkSize[1], z_dim);
  dim3 grid (globalWorkSize[0] / localWorkSize[0], globalWorkSize[1] / localWorkSize[1], 64);
  ...
  dim3 grid2 (globalWorkSize[0] / localWorkSize[0], 192);
