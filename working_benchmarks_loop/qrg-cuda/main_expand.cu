...
{
  unsigned int globalID_x   = blockIdx.x%2 == 0 ? (blockIdx.x/2) * blockDim.x + threadIdx.x : (blockIdx.x/2) * blockDim.x + (gridDim.x * blockDim.x / 2) + threadIdx.x;
...
{
  const unsigned int globalID   = blockIdx.x%2 == 0 ? (blockIdx.x/2) * blockDim.x + threadIdx.x : blockIdx.x/2  * blockDim.x + (gridDim.x * blockDim.x / 2) + threadIdx.x;
...
int main(int argc, const char **argv)
{
  ...
  dim3 grid2 (2*globalWorkSize[0] / localWorkSize[0]);
