...
__global__ void VoteAnyKernel1(const unsigned int *input, unsigned int *result,
  ...
  int iter_start = (blockIdx.x) * (repeat / 2); // Allow for some divergence in the loop
  int iter_end =  (1+blockIdx.x) * (repeat / 2); // Ensure we loop through the correct number of times
  //int total_iters= 0;
  ...
  //printf("total_iters=%d, blockIdx.x=%d, threadIdx.x=%d\n", total_iters, blockIdx.x, tx);
}
