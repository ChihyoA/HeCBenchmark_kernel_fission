...
// non-zero value
__global__ void VoteAnyKernel1_split(const unsigned int *input, unsigned int *result,
                               int repeat, bool stream_A) {
  int tx = threadIdx.x;

  // 나눠진 index 범위 정의
  const size_t split_start = stream_A ? 0 : (repeat / 2);
  const size_t split_end   = stream_A ? (repeat / 2) : repeat;
...
// value
__global__ void VoteAllKernel2_split(const unsigned int *input, unsigned int *result,
                               int repeat, bool stream_A) {
  int tx = threadIdx.x;
