...
int main(int argc, char **argv) {
  ...
  // Warmup
  for (int i = 0; i < n_way_split; ++i) {
  VoteAnyKernel1_split<<<gridBlock, threadBlock, 0, streams[i]>>>(d_input, d_result, repeat, i, n_way_split);
  }
  ...
  // Warmup
  for (int i = 0; i < n_way_split; ++i) {
  VoteAllKernel2_split<<<gridBlock, threadBlock, 0, streams[i]>>>(d_input, d_result, repeat, i, n_way_split);
  }
