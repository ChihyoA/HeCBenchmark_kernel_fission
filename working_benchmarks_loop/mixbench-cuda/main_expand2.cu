                               ...
                               const int compute_iterations)
{
  ...
  int blockidx_x = blockIdx.x >= gridDim.x /2 ? blockIdx.x - gridDim.x /2 : blockIdx.x;
  int idx = blockidx_x*blockSize*granularity + threadIdx.x;
  const int big_stride = gridDim.x/2*blockSize*granularity;

  float tmps[granularity];

  int iter_start = blockIdx.x >= gridDim.x /2 ? fusion_degree / 2 : 0;
  int iter_end = blockIdx.x >= gridDim.x /2 ? fusion_degree: fusion_degree / 2;
...
void mixbenchGPU(long size, int repeat) {
  ...
  const int grid_dim = reduced_grid_size/block_dim*2;
