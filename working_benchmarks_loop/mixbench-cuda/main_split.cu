...
__global__ void benchmark_func_split(float *g_data, const int blockdim,
                               const int compute_iterations, const bool stream_A)
{
  ...
  // 나눠진 index 범위 정의
  const size_t split_start = stream_A ? 0 : (fusion_degree / 2);
  const size_t split_end   = stream_A ? (fusion_degree / 2) : fusion_degree;
...
void mixbenchGPU(long size, int repeat) {
  ...
  for (int i = 0; i < repeat; i++) {
    benchmark_func_split<<<grid_dim, block_dim, 0, stream1>>>(d_cd, block_dim, i, true);
    benchmark_func_split<<<grid_dim, block_dim, 0, stream2>>>(d_cd, block_dim, i, false);
  }
