...
int main(int argc, char** argv) {
  ...
  cudaEvent_t eventA[(repeat+1)];  // NUM = number of (A,B) pairs
  cudaEvent_t eventB[(repeat+1)];  // NUM = number of (A,B) pairs
  for (int i = 0; i < (repeat+1); ++i) {
      cudaEventCreate(&eventA[i]);
      cudaEventCreate(&eventB[i]);
  }
  cudaEventRecord(eventA[0], stream1);
  cudaEventRecord(eventB[0], stream2);

  for (i = 0; i < repeat; i++)
  {
    cudaStreamWaitEvent(stream1, eventB[i], 0);
    
    genScoreKernel<<<grid, threads, 0, stream1>>>(sizepernode, D_localscore, D_data, D_LG, true);

    cudaEventRecord(eventA[i+1], stream1);
    cudaStreamWaitEvent(stream2, eventA[i], 0);
    genScoreKernel<<<grid, threads, 0, stream2>>>(sizepernode, D_localscore, D_data, D_LG, false);
    cudaEventRecord(eventB[i+1], stream2);
