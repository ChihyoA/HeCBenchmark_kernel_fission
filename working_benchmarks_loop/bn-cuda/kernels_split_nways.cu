                               ...
                               const int num_streams)
{
  ...
  if(id<sizepernode){
    ...
    // 나눠진 index 범위 정의
    const int split_start = stream_A * (NODE_N / num_streams);
    const int split_end   = stream_A+1 == num_streams? NODE_N : (stream_A + 1) * (NODE_N / num_streams);

    for(node=split_start;node<split_end;node++){
