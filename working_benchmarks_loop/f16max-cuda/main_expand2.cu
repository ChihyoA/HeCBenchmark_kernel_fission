          ...
          const size_t size)
{
  ...
  const size_t split_start = blockIdx.x >= gridDim.x / 2 ? 0 : (size / 2);
  const size_t split_end   = blockIdx.x >= gridDim.x / 2 ? (size / 2) : size;
  const size_t block_index = blockIdx.x >= gridDim.x / 2 ? blockIdx.x - gridDim.x / 2 : blockIdx.x;
  const size_t grid_dim = gridDim.x / 2;
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < repeat; i++)
    hmax<half2><<<NUM_OF_BLOCKS*2, NUM_OF_THREADS>>>(
      d_a, d_b, d_r, size);
  ...
  for (int i = 0; i < repeat; i++)
    hmax<half><<<NUM_OF_BLOCKS*2, NUM_OF_THREADS>>>(
