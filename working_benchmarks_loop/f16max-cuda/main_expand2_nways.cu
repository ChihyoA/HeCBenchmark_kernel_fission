...
{
  const size_t original_griddim = gridDim.x / num_streams;
  const size_t block_index = blockIdx.x % original_griddim;
  const size_t block_chunk_idx = blockIdx.x / original_griddim;

  const size_t split_start = block_chunk_idx * (size / num_streams);
  const size_t split_end   = block_chunk_idx + 1 == num_streams ? size : (block_chunk_idx + 1)* (size / num_streams);
  
  const size_t grid_dim = gridDim.x / num_streams;

  for (size_t i = threadIdx.x + blockDim.x * block_index + split_start; 
              i < split_end; i += blockDim.x * grid_dim)
...
void generateInput(half2 * a, size_t size)
{
  for (size_t i = 0; i < size; ++i)
  {
    float x = static_cast<float>(rand() % 1001); // 제한된 범위 사용
    float y = static_cast<float>(rand() % 1001);
    a[i] = __floats2half2_rn(x, y);  // 올바른 half2 생성 방식
  }
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < repeat; i++)
    hmax<half2><<<NUM_OF_BLOCKS*n_way_split, NUM_OF_THREADS>>>(
      d_a, d_b, d_r, size, n_way_split);
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda f16max-cuda main.cu,0);
  ...
  for (int i = 0; i < repeat; i++)
    hmax<half><<<NUM_OF_BLOCKS*n_way_split, NUM_OF_THREADS>>>(
      (half*)d_a, (half*)d_b, (half*)d_r, size*2, n_way_split);
  cudaDeviceSynchronize();
  ...
  time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda f16max-cuda main.cu,1);
  printf("Average kernel execution time %f (us)\n", (time * 1e-3f) / repeat);
