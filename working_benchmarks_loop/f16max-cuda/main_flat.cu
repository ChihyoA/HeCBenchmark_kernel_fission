          ...
          const size_t size)
{
  ...
  //            i < size; i += blockDim.x * gridDim.x)
   ...
   size_t id_y = blockIdx.y * blockDim.x * gridDim.x;
   size_t i = id_x + id_y;
...
int main(int argc, char *argv[])
{
  ...
  dim3 dimGrid(NUM_OF_BLOCKS, loop_idx, 1);
 
  for (int i = 0; i < repeat; i++)
    hmax<half2><<<dimGrid, NUM_OF_THREADS>>>(
      d_a, d_b, d_r, (size / loop_idx));
  ...
  for (int i = 0; i < repeat; i++)
    hmax<half><<<dimGrid, NUM_OF_THREADS>>>(
