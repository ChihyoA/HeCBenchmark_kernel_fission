...
{
  // 나눠진 index 범위 정의
  const int split_start = stream_A * (size / num_streams);
  const int split_end = stream_A + 1 == num_streams ? size : (stream_A + 1)* (size / num_streams);
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < repeat; i++)
  {
    for (int j = 0; j < n_way_split; ++j) {
      hmax<half2><<<NUM_OF_BLOCKS, NUM_OF_THREADS, 0, streams[j]>>>(d_a, d_b, d_r, size, j, n_way_split);
    }
    for (int j = 0; j < n_way_split; ++j) {
  ...
  {
    for (int j = 0; j < n_way_split; ++j) {
    hmax<half><<<NUM_OF_BLOCKS, NUM_OF_THREADS, 0, streams[j]>>>((half*)d_a, (half*)d_b, (half*)d_r, size*2, j, n_way_split);
    }
  }
  cudaDeviceSynchronize();
