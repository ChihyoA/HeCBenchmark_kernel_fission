...
{
  // 나눠진 index 범위 정의
  const size_t split_start = stream_A ? 0 : (size / 2);
  const size_t split_end   = stream_A ? (size / 2) : size;

  for (size_t i = threadIdx.x + blockDim.x * blockIdx.x+split_start; 
              i < split_end; i += blockDim.x * gridDim.x)
              {
                //printf("a[i]: %f, b[i]: %f\n", __half22float2(a[i]), __half22float2(b[i]));
...
int main(int argc, char *argv[])
{
  ...
  {
    hmax<half2><<<NUM_OF_BLOCKS, NUM_OF_THREADS, 0, stream1>>>(d_a, d_b, d_r, size, true);
    hmax<half2><<<NUM_OF_BLOCKS, NUM_OF_THREADS, 0, stream2>>>(d_a, d_b, d_r, size, false);
  }
  ...
  for (int i = 0; i < repeat; i++)
  {
    hmax<half><<<NUM_OF_BLOCKS, NUM_OF_THREADS, 0, stream1>>>((half*)d_a, (half*)d_b, (half*)d_r, size*2, true);
    hmax<half><<<NUM_OF_BLOCKS, NUM_OF_THREADS, 0, stream2>>>((half*)d_a, (half*)d_b, (half*)d_r, size*2, false);
  }
  cudaDeviceSynchronize();
